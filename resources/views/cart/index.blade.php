...
<style>
    ...
    .cart-table th:nth-child(1), .cart-table td:nth-child(1) { width: 24%; min-width: 170px; }
    .cart-table th:nth-child(2), .cart-table td:nth-child(2) { width: 10%; min-width: 88px; }
    .cart-table th:nth-child(3), .cart-table td:nth-child(3) { width: 12%; min-width: 88px; }
    .cart-table th:nth-child(4), .cart-table td:nth-child(4) { width: 8%; min-width: 64px; }
    .cart-table th:nth-child(5), .cart-table td:nth-child(5) { width: 10%; min-width: 84px; }
    .cart-table th:nth-child(6), .cart-table td:nth-child(6) { width: 14%; min-width: 100px; }
    ...
    .cart-table th:nth-child(7), .cart-table td:nth-child(7) { width: 6%; text-align: center; }
    ...
    [dir="rtl"] .cart-row td:first-child { border-radius: 0 15px 15px 0; }
    .cart-row td:last-child { border-radius: 0 15px 15px 0; }
    [dir="rtl"] .cart-row td:last-child { border-radius: 15px 0 0 15px; }
    ...
    .code-select:focus { border-color: var(--elx-cyan); }
    [dir="rtl"] .code-select {
            ...
            <div class="cart-layout-grid">
                        ...
                        <table class="cart-table">
                            ...
                            <tbody>
                                @foreach($cartLines as $line)
                                    <tr class="cart-row @if($line['is_unavailable']) cart-row--unavailable @endif" data-id="{{ $line['id'] }}" data-max-qty="{{ $line['max_quantity'] }}">
                                        <td>
                                            <div class="cart-product-cell">
                                                <div class="cart-product-thumb">
                                                    @if(!empty($line['image']))
                                                        <img src="{{ asset('storage/' . $line['image']) }}" alt="{{ $line['display_name'] }}">
                                                    @else
                                                        <div style="width: 100%; height: 100%; background: #1a2e38; display: flex; align-items: center; justify-content: center; color: var(--elx-cyan); font-size: 1.5rem;">
                                        ...
                                        <td class="cart-size-cell" data-label="{{ __('cart_page.size') }}">
                                            @if(!empty($line['variant_options']))
                                                <form method="POST" action="{{ route('cart.update-variant') }}" class="cart-variant-form">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="hidden" name="id" value="{{ $line['id'] }}">
                                                    <select name="country_price_id" class="cart-size-select" aria-label="{{ __('cart_page.size') }}" onchange="this.form.submit()">
                                                        @foreach($line['variant_options'] as $option)
                                                            <option value="{{ $option['id'] }}" @selected((int) $line['current_variant_id'] === (int) $option['id']) @disabled($option['stock'] <= 0)>
                                                                {{ $option['label'] }}@if($option['stock'] <= 0) ({{ __('shop.out_of_stock') }})@endif
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </form>
                                            ...
                                            @else
                                                <span class="cart-size-label" style="color: var(--elx-gray); font-weight: 500;">—</span>
                                            @endif
                                        </td>
                                        <td class="cart-col-price" data-label="{{ __('cart_page.price') }}">
                                            {{ $cartPricing->formatCompactPrice($line['price'], $line['currency_symbol']) }}
                                        </td>
                                        <td class="cart-col-points" data-label="{{ __('cart_page.points') }}">{{ $line['points'] }}</td>
                                        ...
                                        <td style="text-align: center;">
                                            <form method="POST" action="{{ route('cart.remove') }}" class="cart-remove-form" onsubmit="return confirm(@json(__('cart_page.remove_confirm')))">
                                                @csrf
                                                @method('DELETE')
                                                <input type="hidden" name="id" value="{{ $line['id'] }}">
                                                <button type="submit" class="remove-btn" aria-label="{{ __('cart_page.remove_confirm') }}">
                ...
                <div data-animate>
                    <div class="cart-card cart-summary-card">
                        ...
                        <div id="delivery-fee-row" style="display: flex; justify-content: space-between; margin-bottom: 1rem; color: var(--elx-gray);">
                        ...
                        <form action="{{ route('checkout') }}" method="POST">
                            ...
                            @php
                                ...
                                $cCode = old('country_code', $defaultPhoneCountry ?? '+966');
                                $pNum = $phone;
                            ...
                            @if(!empty($deliveryCityOptions))
                                <label for="delivery_city_id" style="display: block; color: var(--elx-light); font-size: 0.85rem; margin-bottom: 0.35rem;">{{ __('cart_page.delivery_city_label') }}</label>
                                <select
                                ...
                                >
                                    ...
                                    @foreach($deliveryCityOptions as $cityOption)
                                        <option
                                            value="{{ $cityOption['id'] }}"
                                            data-fee="{{ $cityOption['delivery_fee'] }}"
                            ...
                            @else
                                <select name="delivery_city_id" id="delivery_city_id" class="form-input" style="display: none; margin-bottom: 1rem;" data-fees="{}">
                                    <option value=""></option>
                                </select>
                            @endif
                            @error('delivery_city_id')<div style="color: #ff8a8a; font-size: 0.8rem; margin-top: -0.5rem; margin-bottom: 1rem;">{{ $message }}</div>@enderror

                            <label for="shared_shipping_order_input" style="display: block; color: var(--elx-light); font-size: 0.85rem; margin-bottom: 0.35rem;">{{ __('cart_page.shared_shipping_order_label') }}</label>
                            <p style="color: var(--elx-gray); font-size: 0.8rem; margin: 0 0 0.5rem; line-height: 1.4;">{{ __('cart_page.shared_shipping_order_hint') }}</p>
                            @php
                            ...
                            <div style="display: flex; gap: 0.5rem; margin-bottom: 0.75rem; align-items: stretch;">
                                <input
                                    type="text"
                                    id="shared_shipping_order_input"
                                    class="form-input"
                                    autocomplete="off"
                                    maxlength="6"
                                    placeholder="{{ __('cart_page.shared_shipping_order_placeholder') }}"
                                    value="{{ $oldSharedReference }}"
                                    style="flex: 1; margin-bottom: 0; text-transform: uppercase; letter-spacing: 0.08em;"
                                >
                                <button
                                    type="button"
                                    id="shared-shipping-check-btn"
                                    class="elx-btn elx-btn--primary"
                                ...
                                >
                                    {{ __('cart_page.shared_shipping_check') }}
                                </button>
                                <button
                                    type="button"
                                    id="shared-shipping-clear-btn"
                                    class="elx-btn"
                            ...
                            <input type="hidden" name="shared_shipping_order_id" id="shared_shipping_order_id" value="{{ old('shared_shipping_order_id') }}">
                            <div id="shared-shipping-status" style="display: none; font-size: 0.85rem; margin-bottom: 1rem; line-height: 1.4;"></div>
                            @error('shared_shipping_order_id')<div style="color: #ff8a8a; font-size: 0.8rem; margin-top: -0.5rem; margin-bottom: 1rem;">{{ $message }}</div>@enderror

                            @php
                                $userCode = auth()->user()?->user_code ?? '';
                                $defaultCode = $userCode ?: ($sponsorCodes->first()?->code ?? '');
                            @endphp
                            <input type="hidden" name="user_code" id="user_code_hidden" value="{{ old('user_code', $defaultCode) }}">
                            
                            <select class="code-select" id="code_select" onchange="handleCodeSelect(this)">
                                @if($userCode)
                                    <option value="{{ $userCode }}" selected>{{ $userCode }} ({{ __('cart_page.my_code') }})</option>
                                @endif
                            ...
                            <div class="code-custom-input" id="code_custom_wrap">
                                <input type="text" class="form-input" id="code_custom_input" placeholder="{{ __('cart_page.enter_code') }}" style="margin-bottom: 0;" oninput="document.getElementById('user_code_hidden').value = this.value.toUpperCase(); this.value = this.value.toUpperCase();">
                            </div>
                            @error('user_code')<div style="color: #ff8a8a; font-size: 0.8rem; margin-top: 0.25rem; margin-bottom: 1rem;">{{ $message }}</div>@enderror
                            ...
                            @auth
                                    ...
                                    <select id="selected_address" class="form-input" style="cursor: pointer; margin-bottom: 0;">
                                        ...
                                        @foreach($userAddresses as $addr)
                                            <option value="{{ $addr->address }}" {{ ($defaultAddress === $addr->address || ($mainAddress && $addr->is_main && !old('address'))) ? 'selected' : '' }}>
                                                {{ \Illuminate\Support\Str::limit($addr->address, 60) }} {{ $addr->is_main ? __('cart_page.main_address') : '' }}
                                            </option>
                                        @endforeach
                                        <option value="__new__">{{ __('cart_page.add_new_address') }}</option>
                                    </select>
                                    <input type="hidden" name="address" id="address" value="{{ $defaultAddress }}">
                                    <input type="text" id="address_new" class="form-input" placeholder="{{ __('cart_page.shipping_address') }}" style="display: none; margin-top: 1rem;">
                                    <div id="new_address_controls" style="margin-top: 0.75rem; margin-bottom: 1rem; color: var(--elx-light); font-size: 0.9rem; display: none;">
                                ...
                                @else
                                    ...
                                    <div id="new_address_controls" style="margin-bottom: 1rem; color: var(--elx-light); font-size: 0.9rem;">
                                        <label><input type="checkbox" name="save_address" value="1" checked> {{ __('cart_page.save_address') }}</label>
                                        &nbsp;&nbsp;
                                        <label><input type="checkbox" name="is_main_address" value="1" checked> {{ __('cart_page.set_main') }}</label>
                                    </div>
                            ...
                            <button type="submit" class="elx-btn elx-btn--primary" style="width: 100%; justify-content: center; padding: 1rem; margin-top: 1rem;" id="checkout-submit-btn" @if($hasUnavailableItems) disabled @endif>
                        ...
                        @endphp
                        <script type="application/json" id="cart-checkout-config">{!! json_encode($cartCheckoutConfig, JSON_UNESCAPED_UNICODE) !!}</script>
                        @php
