...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="mb-1 fw-bold text-dark">Special Requests</h2>
            <p class="text-muted mb-0">Custom demands and private purchase offers for your products</p>
        </div>
    </div>

    <div class="card border-0 shadow-sm" style="border-radius: 16px;">
        <div class="card-header bg-white border-0 py-3 ps-4">
            <h5 class="card-title fw-bold m-0 text-dark">Pending & Notified Customer Requests</h5>
        </div>
                        ...
                        @forelse($specialRequests as $request)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <span class="fw-bold">{{ $request->name ?? 'Guest User' }}</span>
                                    @endif
                                </td>
                                <td>
                                    @php
                                ...
                                <td>
                                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $request->phone) }}?text={{ urlencode('Hello! The product (' . ($request->item ? $request->item->name : 'you requested') . ') is now available at Elixira!') }}" target="_blank" class="btn btn-sm btn-success px-3 py-1.5 fw-medium" style="border-radius: 6px;">
                                        <i class="fab fa-whatsapp me-1"></i> Chat ({{ $request->phone }})
                                    </a>
                                </td>
                                <td>
                                    @if($request->email)
                                ...
                                <td>
                                    @if($request->status === 'pending')
                                        <span class="badge bg-warning text-dark py-1.5 px-3 rounded-pill" style="font-size: 0.75rem;">Pending</span>
                                    @else
                                        <span class="badge bg-success py-1.5 px-3 rounded-pill" style="font-size: 0.75rem;">Notified</span>
                                    @endif
                                </td>
                                <td>
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center py-5 text-muted">
                                    ...
                                    <p class="mb-0">No special requests for your products yet.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
