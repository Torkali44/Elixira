...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <h1 class="h3 mb-4 text-gray-800">{{ __('vendor.reviews_page.title') }}</h1>

             <div class="mb-4">
                <a href="{{ route('admin.reviews.create') }}" class="btn btn-primary">{{ __('vendor.reviews_page.add_new') }}</a>
            </div>
                        ...
                        <table class="table table-bordered table-hover align-middle" style="color: #333;">
                            <thead class="bg-primary text-white">
                                <tr>
                                    ...
                                    <th style="width: 120px;">{{ __('vendor.reviews_page.col_rating') }}</th>
                                    <th>{{ __('vendor.reviews_page.col_reviewer') }}</th>
                                    <th>{{ __('vendor.reviews_page.col_testimonial') }}</th>
                                    <th style="width: 100px;">{{ __('vendor.reviews_page.col_status') }}</th>
                                    <th style="width: 140px;">{{ __('vendor.reviews_page.col_actions') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr style="background-color: #fff;">
                                    ...
                                    <td>
                                        <div class="text-warning">
                                            ...
                                            @else
                                                <span class="text-muted small">{{ __('vendor.reviews_page.na') }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td>
                                        ...
                                        @else
                                            <div class="text-muted small font-italic">{{ __('vendor.reviews_page.admin_entry') }}</div>
                                        @endif
                                    </td>
                                    <td>
                                        @if($review->type === 'direct')
                                        ...
                                        @elseif($review->type === 'video')
                                            <a href="{{ $review->content }}" target="_blank" class="btn btn-sm btn-outline-danger">
                                                <i class="fab fa-youtube mr-1"></i> {{ __('vendor.reviews_page.video_link') }}
                                            </a>
                                        @else
                                    ...
                                    <td class="text-center">
                                        @if($review->status === 'approved')
                                            <span class="badge" style="background-color: rgba(40, 167, 69, 0.1); color: #28a745; border: 1px solid #28a745; padding: 5px 10px; border-radius: 50px; font-weight: bold;">
                                                <i class="fas fa-check-circle mr-1"></i> {{ __('vendor.reviews_page.approved') }}
                                            </span>
                                        @elseif($review->status === 'pending')
                                            <span class="badge" style="background-color: rgba(255, 193, 7, 0.1); color: #856404; border: 1px solid #ffc107; padding: 5px 10px; border-radius: 50px; font-weight: bold;">
                                                <i class="fas fa-clock mr-1"></i> {{ __('vendor.reviews_page.pending') }}
                                            </span>
                                        @else
                                            <span class="badge" style="background-color: rgba(220, 53, 69, 0.1); color: #dc3545; border: 1px solid #dc3545; padding: 5px 10px; border-radius: 50px; font-weight: bold;">
                                                <i class="fas fa-times-circle mr-1"></i> {{ __('vendor.reviews_page.rejected') }}
                                            </span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="d-flex justify-content-center gap-1">
                                            <form action="{{ route('admin.reviews.updateStatus', $review->id) }}" method="POST" class="d-inline">
                                                ...
                                                @method('PATCH')
                                                @if($review->status !== 'approved')
                                                    <button type="submit" name="status" value="approved" class="btn btn-sm btn-success p-1" title="{{ __('vendor.reviews_page.approve') }}">
                                                        <i class="fas fa-check"></i>
                                                    </button>
                                                @endif
                                                @if($review->status !== 'rejected')
                                                    <button type="submit" name="status" value="rejected" class="btn btn-sm btn-warning p-1" title="{{ __('vendor.reviews_page.reject_btn') }}">
                                                        <i class="fas fa-ban"></i>
                                                    </button>
                                                @endif
                                            </form>
                                            
                                            <form action="{{ route('admin.reviews.destroy', $review->id) }}" method="POST" class="d-inline ml-1" data-confirm="{{ __('vendor.reviews_page.confirm_delete') }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-danger p-1" title="{{ __('vendor.reviews_page.delete') }}">
